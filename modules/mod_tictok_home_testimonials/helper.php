...
class modpagetestimonials
{
    ...
    public function getHtml($categoryid)
    {
        ...
        $query = $db->getQuery(true)
            ->select($db->quoteName(array('c.id', 'c.title', 'c.introtext', 'c.images','ca.title' ), array('id', 'title', 'introtext', 'images', 'catitle')))
            ...
            ->where($db->quoteName('ca.parent_id') . '='.$db->Quote($categoryid) . ' AND ' . $db->quoteName('c.state') . '=1' )
